...
# [설정] 파라미터
# ============================================================
TRIGGER_ARM_DIST = 0.10   # 정지점(=start_zone) 10cm 이내일 때만 트리거 인정
MIN_HOLD_SPEED   = 0.0    # 실차면 0.0 권장. (지금 -0.0001, -0.03 같은 음수는 미세하게 밀릴 수 있음)
...
# ============================================================
HOLD_BRAKE_VEL = 0.0          # HV 트리거 대기 중 계속 보낼 속도(약한 역토크)
    ...
    def _callback_pose(self, msg):
        ...
        if not self.stop_logic_disabled:
            ...
            if dist_to_start < stop_zero_dist:
                # 1) 정지선까지는 무조건 멈추러 간다 (기본은 0으로 고정)
                target_vel_req = STOP_VELOCITY

                # 2) 정지선(목표점) 근처에 도착했을 때만 트리거 확인
                if dist_to_start < TRIGGER_ARM_DIST:
                    if self._check_hv_in_zone(self.start_trigger_points):
                        ...
                        self.wait_hold_active = False
                        target_vel_req = TARGET_VELOCITY
                        print(f"[START] V{self.id_str} Triggered at stop-line (<{TRIGGER_ARM_DIST:.2f}m)!", end='\r')
                    else:
                        # 트리거 없으면 "정지선에서만" hold 사용
                        ...
                        move_dist = math.hypot(
                            self.curr_x - self.wait_hold_start_pos[0],
                            self.curr_y - self.wait_hold_start_pos[1]
                        )

                        # 안전장치: 밀리면 hold 끄고 0 유지
                        if move_dist > HOLD_MAX_MOVE_DIST:
                            target_vel_req = STOP_VELOCITY
                            print(f"[WAIT] V{self.id_str} HOLD LIMIT -> STOP", end='\r')
                        else:
                            # 실차면 음수 hold 대신 0 추천
                            target_vel_req = MIN_HOLD_SPEED
                            print(f"[WAIT] V{self.id_str} Stop-line hold (0.0)", end='\r')
                else:
                    # 3) 정지선까지 아직 멀면, 완전정지가 아니라 '아주 천천히' 끌고가도 됨 (선택)
                    #    (이게 있으면 'zone 경계에서 갑자기 뚝'이 줄어듦)
                    target_vel_req = max(STOP_MIN_VEL, STOP_VELOCITY)

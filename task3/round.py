...
class Problem3DualZoneGuardianMux(Node):
    ...
    def tick(self):
        ...
        # 4. Publish Commands (이 부분을 찾아서 수정!)
        for vid in self.VEH_IDS:
            use_round = (self.pose[vid] is not None) and self._in_round_box(self.pose[vid])
            src = self.round_raw[vid] if use_round else self.raw[vid]
            ...
            # ==========================================================
            # ★ [긴급 패치] 강제 주행 모드 (God Mode)
            # 모든 로직 무시하고 MapPredictionDriver의 조향 + 고정 속도 사용
            # ==========================================================
            
            # 1) 조향: MapPredictionDriver가 계산한 값(self.raw)을 강제로 씀 
            #    (회전교차로 로직인 round.py가 조향하는걸 막고, 우리가 수집하려는 로직으로 주행)
            steer = float(self.raw[vid].angular.z) 

            # 2) 속도: 0.4로 강제 고정 (HV 대기 로직 무시)
            out_v = 0.4 

...
class MapPredictionDriver(Node):
    ...
    def drive_loop(self):
        ...
        # ===========================
        # jump detection (global relocalize)
        # ===========================
        actual_dist = math.hypot(
            self.path_x[self.last_idx] - self.curr_x,
        ...
        # 5) publish raw
        cmd = Accel()
        cmd.linear.x = float(self.current_vel_cmd)   # 0.7 fixed
        cmd.angular.z = float(final_steer)
        self.accel_raw_pub.publish(cmd)


# ============================================================
# [Guardian + Mux]  (TOP + BOTTOM + CROSS + (ADD) Start/Exit HV Safety)
# ============================================================
class Problem3DualZoneGuardianMux(Node):
    def __init__(self):
        ...
        # ✅ 여기 MIN_SPEED는 "정지 로직"이 있으니 STOP_VELOCITY까지 허용해야 함
        ...
        # ============================================================
        # [FOUR-WAY INTERSECTION] settings
        # ============================================================
        ...
        self.FW_EPS = 0.001
        self.FW_V_NOM = 0.7
        self.FW_RANK_SPEEDS_2P = [0.7, 0.1]
        self.FW_RANK_SPEEDS_3P = [0.7, 0.4, 0.1, 0.1]
        ...
        # HV 구독 (Start/Exit safety가 있으니 무조건 달아둠)
        ...
        for vid in self.VEH_IDS:
            ...
            if cfg.get("start_trigger"):
                self.get_logger().info(f"[SAFE] CAV{vid}: Start Trigger Loaded ({len(cfg['start_trigger'])} pts)")
            if cfg.get("out_zone") and cfg.get("danger_zone"):
                self.get_logger().info(f"[SAFE] CAV{vid}: Exit Conflict Logic Activated")

        # timer
        self.create_timer(self.TICK, self.tick)
        self.get_logger().info("✅ GuardianMux: zones(TOP/BOT/FW) + HV Start/Exit Safety merged")

    # ============================================================
    # [ADD] HV callbacks
    # ============================================================
    def _cb_hv19(self, msg):
        self.hv19 = (float(msg.pose.position.x), float(msg.pose.position.y))
        self.hv19_active = True

    def _cb_hv20(self, msg):
        self.hv20 = (float(msg.pose.position.x), float(msg.pose.position.y))
    ...
    def _apply_limit_ramp(self, vid, tgt, force_immediate=False):
        """
        force_immediate=True면 (예: HV safety STOP) 램프 무시하고 즉시 세팅
        """
        if tgt is None:
    ...
    def _hv_in_points(self, pts):
        if not pts:
            return False, None  # (hit?, which_hv)
        # hv19
        if self.hv19_active and self.hv19 is not None:
            hx, hy = self.hv19
            for x, y in pts:
                if math.hypot(x - hx, y - hy) < self.HV_DETECT_RADIUS:
                    return True, 19
        # hv20
        if self.hv20_active and self.hv20 is not None:
            hx, hy = self.hv20
            for x, y in pts:
                if math.hypot(x - hx, y - hy) < self.HV_DETECT_RADIUS:
                    return True, 20
    ...
    def _compute_hv_safety_limit(self, vid):
        """
        return:
          - (limit_value or None, force_immediate(bool), reason_str or None)
        정책(첫 코드 그대로):
          1) Start Zone: zone 안이면 일단 STOP, trigger 들어오면 GO(latch), reset_distance 벗어나면 latch 해제
          2) Exit/Conflict: out_zone 안이고 danger_zone에 HV 있으면 STOP (Start에서 GO라도 STOP이 우선)
        """
        cfg = self.safety_cfg.get(vid, None)
        if cfg is None:
            return None, False, None

        # ---- Start logic ----
        start_zone = cfg.get("start_zone", [])
        start_trigger = cfg.get("start_trigger", [])
        dist_to_start = self._min_dist_to_points(vid, start_zone)

        if cfg["stop_logic_disabled"]:
            if dist_to_start > self.RESET_DISTANCE:
                cfg["stop_logic_disabled"] = False
        else:
            if dist_to_start < self.ZONE_RADIUS:
                # 기본 STOP
                hit, which = self._hv_in_points(start_trigger)
                if hit:
                    cfg["stop_logic_disabled"] = True
                    # GO → limit None
                else:
                    # STOP 강제
                    return self.STOP_VELOCITY, True, f"START_WAIT(HV none)"

        # ---- Exit/Conflict logic ----
        out_zone = cfg.get("out_zone", [])
        danger_zone = cfg.get("danger_zone", [])
        if out_zone and danger_zone:
            dist_to_out = self._min_dist_to_points(vid, out_zone)
            if dist_to_out < self.ZONE_RADIUS:
                hit, which = self._hv_in_points(danger_zone)
                if hit:
                    return self.STOP_VELOCITY, True, f"EXIT_YIELD(HV_{which})"
    ...
    def tick(self):
        ...
        fw_in_eff = [
            vid for vid in self.VEH_IDS
            if (self.pose[vid] is not None) and self.fw["active"][vid] and self.fw["approaching"][vid]
        ]
        ...
        if fw_targets:
            ...
            fw_targets_sorted = sorted(
                fw_targets,
                key=lambda v: (self._fw_dist(v) if self._fw_dist(v) is not None else 1e9)
            )
        ...
        # ============================================================
        # [ADD] HV Start/Exit Safety limits
        # ============================================================
        hv_limits = {vid: None for vid in self.VEH_IDS}
        hv_force = {vid: False for vid in self.VEH_IDS}
        hv_reason = {vid: None for vid in self.VEH_IDS}

        for vid in self.VEH_IDS:
            lim, force_now, reason = self._compute_hv_safety_limit(vid)
            hv_limits[vid] = lim
            hv_force[vid] = force_now
            hv_reason[vid] = reason

        # 3) merge limits (min across zones + hv safety)
        for vid in self.VEH_IDS:
            cands = []
            ...
            if hv_limits.get(vid) is not None:
                cands.append(float(hv_limits[vid]))

            self.tgt_limit[vid] = min(cands) if cands else None

        # 4) apply ramp (HV safety STOP은 즉시)
        for vid in self.VEH_IDS:
        ...
        if self._log_counter % 20 == 0:
            print(f"[GUARD] TOP_on={top_on} eff={top_eff} | BOT_on={bot_on} eff={bot_eff} | FW_eff={fw_in_eff}")
            for vid in self.VEH_IDS:
                extra = f" | HV={hv_reason[vid]}" if hv_reason[vid] else ""
                print(
                    f"  {vid}: raw={float(self.raw[vid].linear.x):.2f} "

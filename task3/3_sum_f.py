...
# ============================================================
# [Guardian + Mux]  (TOP + BOTTOM) 0.7-fixed driving version
# - Paste this BELOW your Driver code
# ============================================================
...
class Problem3DualZoneGuardianMux(Node):
    def __init__(self):
        ...
        # 3대 이상: 1등/2등/3등/그외
        self.RANK_SPEEDS_3P = [0.7, 0.45, 0.2, 0.1]

        # 2대: 1등/2등
        self.RANK_SPEEDS_2P = [0.7, 0.3]
        ...
        # 사지교차로 반경 (너가 테스트하면서 조정하면 됨)
        self.FW_RADIUS = 2.0
        self.FW_EXIT_RADIUS = 0.4
        self.FW_HYSTERESIS_N = 10

        # 접근판정(거리 감소) 파라미터: 위/아래 교차로에서 쓰던 걸 재사용해도 되고, 별도로 둬도 됨
        self.FW_APPROACH_N = self.APPROACH_N
        self.FW_EPS = self.EPS

        # 사지교차로 속도 정책 (지금 너가 말한 고정속도 0.7 기준)
        self.FW_V_NOM = 0.7
        ...
        # __init__ 내부에 추가
        self.pose = {vid: None for vid in self.VEH_IDS} # (x, y) 중심점 유지
        self.yaw = {vid: 0.0 for vid in self.VEH_IDS}   # 헤딩 방향 별도 저장
        self.WHEELBASE = 0.211
        self.DIST_HALF_L = self.WHEELBASE / 2.0
        # ✅ 네가 정리한 10가지 케이스(부분집합 매칭)
    ...
    # Problem3DualZoneGuardianMux 클래스 내부에 추가
    def _get_front_pos(self, vid):
        p = self.pose[vid]  # (x, y)
        if p is None: return None
        
        curr_yaw = self.yaw[vid] # float
        
        # 삼각함수를 이용한 전륜 좌표 천이
        x_f = p[0] + self.DIST_HALF_L * math.cos(curr_yaw)
        y_f = p[1] + self.DIST_HALF_L * math.sin(curr_yaw)
        return (x_f, y_f)
    # ============================================================
    ...
    def _make_pose_cb(self, vid):
        def cb(msg):
    # 1. 중심 좌표 (x, y)는 기존 형식을 유지 (float, float)
    ...
    def _fw_dist(self, vid):
        f_pos = self._get_front_pos(vid) # 전륜 좌표 획득
        if f_pos is None: return None
        return math.hypot(f_pos[0] - self.FW_CENTER[0], f_pos[1] - self.FW_CENTER[1])
    def _fw_get_direction(self, vid):
        f_pos = self._get_front_pos(vid) # 전륜 좌표 획득
        if f_pos is None: return None
        
        dx = f_pos[0] - self.FW_CENTER[0]
        dy = f_pos[1] - self.FW_CENTER[1]
    ...
    def _compute_zone_limits(self, zone_name):
        ...
        for i, vid in enumerate(rank_list):
            ...
            # 1등(0.7)은 굳이 제한 걸 필요 없음(드라이버 raw가 0.7이라면)

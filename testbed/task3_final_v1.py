...
# ============================================================
HARD_PARAMS = {
    "vel": 0.90, "look_ahead": 0.60, "kp": 6.0, "ki": 0.045, "kd": 1.0, "k_cte": 3.0
}
EASY_PARAMS = {
    "vel": 1.5, "look_ahead": 0.65, "kp": 6.0, "ki": 0.05, "kd": 1.0, "k_cte": 3.0
}
STRAIGHT_PARAMS = {
    "vel": 1.6, "look_ahead": 1.2, "kp": 2.0, "ki": 0.002, "kd": 0.5, "k_cte": 1.0
} ### change straight vel 1.8 -> 1.6

WHEELBASE = 0.211
DIST_CENTER_TO_REAR = WHEELBASE / 2.0
TICK_RATE = 0.02      # 50 Hz
ACCEL_LIMIT = 3.0    ### change  1.5 -> 3.0
DECEL_LIMIT = 2.5
...
class MapPredictionDriver(Node):
    def __init__(self, vehicle_id: int, path_filename: str):
        ...
        self.current_vel_cmd = 1.5  ## ### change 1.5 -> 1.0 줄이기
        self.mode = "HARD"
    ...
    def drive_loop(self):
        ...
        if math.hypot(path_dx, path_dy) < 0.001:  ###원래 0.02였음
            cte = 0.0
        else:
        ...
        if in_zone:
            ...
            if abs(final_steer) > 0.90:
                ...
                if self.mode == "STRAIGHT":
                    ...
                    if filter_val < 0.22: next_mode = "STRAIGHT"  ### change 0.15 -> 0.22
...
class Problem3DualZoneGuardianMux(Node):
    def __init__(self):
        ...
        self.RANK_SPEEDS_3P = [MAX_SPEED, 0.8, 0.1, 0.1]   ### change 0.8 0.1 0.1
        self.RANK_SPEEDS_2P = [MAX_SPEED, 0.1] ### change 0.1
        ...
        self.APPROACH_N = 3
        self.EPS = 0.0002   ### change 0.0002
        self.HYSTERESIS_N = 5

        self.TICK = 0.02
        self.RAMP_DOWN_PER_SEC = 3.0  ### change 3.0
        self.RAMP_UP_PER_SEC = 2.1 ### change 2.1
        ...
        # HOLD (감속 유지) 설정
        # =========================
        self.HOLD_TICKS = 10  # 감속 유지 시간 (tick 단위)
        ...
        self.FW_EPS = 0.0002 ### change 0.0002
        self.FW_V_NOM = MAX_SPEED
        self.FW_RANK_SPEEDS_2P = [MAX_SPEED, 0.1] ### change 0.1
        self.FW_RANK_SPEEDS_3P = [MAX_SPEED, 0.8, 0.1, 0.1] ### change 0.8 0.1 0.1
        self.fw = {
